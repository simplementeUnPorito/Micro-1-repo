...
***************************************/
static void POT_CalcGain(uint8 resolution);


/***************************************
* Global data allocation
***************************************/
uint8 POT_initVar = 0u;
volatile int16 POT_offset;
volatile int16 POT_countsPerVolt;     /* Obsolete Gain compensation */
volatile int32 POT_countsPer10Volt;   /* Gain compensation */
volatile int16 POT_shift;


/*******************************************************************************
* Function Name: POT_Start
********************************************************************************
*
* Summary:
*  This is the preferred method to begin component operation.
*  POT_Start() sets the initVar variable, calls the
*  POT_Init() function, and then calls the
*  POT_Enable() function.
*
* Parameters:
*  None.
...
*
* Global variables:
*  The POT_initVar variable is used to indicate when/if initial
*  configuration of this component has happened. The variable is initialized to
*  zero and set to 1 the first time ADC_Start() is called. This allows for
*  component Re-Start without re-initialization in all subsequent calls to the
*  POT_Start() routine.
*  If re-initialization of the component is required the variable should be set
*  to zero before call of POT_Start() routine, or the user may call
*  POT_Init() and POT_Enable() as done in the
*  POT_Start() routine.
*
* Side Effect:
...
*
* Global variables:
*  The POT_offset variable is initialized to 0.
*
*******************************************************************************/
void POT_Init(void)
{
    ...
    #if(POT_DEFAULT_INTERNAL_CLK)
        POT_theACLK_SetMode(CYCLK_DUTY);
    #endif /* End POT_DEFAULT_INTERNAL_CLK */

    #if(POT_IRQ_REMOVE == 0u)
        /* Start and set interrupt vector */
        CyIntSetPriority(POT_INTC_NUMBER, POT_INTC_PRIOR_NUMBER);
        (void)CyIntSetVector(POT_INTC_NUMBER, &POT_ISR);
    #endif   /* End POT_IRQ_REMOVE */

    /* Enable IRQ mode*/
    POT_SAR_CSR1_REG |= POT_SAR_IRQ_MASK_EN | POT_SAR_IRQ_MODE_EDGE;

    /*Set SAR ADC resolution ADC */
    POT_SetResolution(POT_DEFAULT_RESOLUTION);
    POT_offset = 0;
}


/*******************************************************************************
* Function Name: POT_Enable
********************************************************************************
*
* Summary:
...
void POT_Enable(void)
{
    ...
    POT_PWRMGR_SAR_REG |= POT_ACT_PWR_SAR_EN;

     /* Enable the SAR ADC in Standby Power Mode*/
    POT_STBY_PWRMGR_SAR_REG |= POT_STBY_PWR_SAR_EN;

    /* This is only valid if there is an internal clock */
    #if(POT_DEFAULT_INTERNAL_CLK)
        POT_PWRMGR_CLK_REG |= POT_ACT_PWR_CLK_EN;
        POT_STBY_PWRMGR_CLK_REG |= POT_STBY_PWR_CLK_EN;
    #endif /* End POT_DEFAULT_INTERNAL_CLK */

    /* Enable VCM buffer and Enable Int Ref Amp */
    tmpReg = POT_SAR_CSR3_REG;
    tmpReg |= POT_SAR_EN_BUF_VCM_EN;
    /* PD_BUF_VREF is OFF in External reference or Vdda reference mode */
    #if((POT_DEFAULT_REFERENCE == POT__EXT_REF) || \
        (POT_DEFAULT_RANGE == POT__VNEG_VDDA_DIFF))
        tmpReg &= (uint8)~POT_SAR_EN_BUF_VREF_EN;
    #else /* In INTREF or INTREF Bypassed this buffer is ON */
        tmpReg |= POT_SAR_EN_BUF_VREF_EN;
    #endif /* POT_DEFAULT_REFERENCE == POT__EXT_REF */
    POT_SAR_CSR3_REG = tmpReg;

    /* Set reference for ADC */
    #if(POT_DEFAULT_RANGE == POT__VNEG_VDDA_DIFF)
        #if(POT_DEFAULT_REFERENCE == POT__EXT_REF)
            POT_SAR_CSR6_REG = POT_INT_BYPASS_EXT_VREF; /* S2 */
        #else /* Internal Vdda reference or obsolete bypass mode */
            POT_SAR_CSR6_REG = POT_VDDA_VREF;           /* S7 */
        #endif /* POT_DEFAULT_REFERENCE == POT__EXT_REF */
    #else  /* Reference goes through internal buffer */
        #if(POT_DEFAULT_REFERENCE == POT__INT_REF_NOT_BYPASSED)
            POT_SAR_CSR6_REG = POT_INT_VREF;            /* S3 + S4 */
        #else /* INTREF Bypassed of External */
            POT_SAR_CSR6_REG = POT_INT_BYPASS_EXT_VREF; /* S2 */
        #endif /* POT_DEFAULT_REFERENCE == POT__INT_REF_NOT_BYPASSED */
    #endif /* VNEG_VDDA_DIFF */

    /* Low non-overlap delay for sampling clock signals (for 1MSPS) */
    #if(POT_HIGH_POWER_PULSE == 0u) /* MinPulseWidth <= 50 ns */
        POT_SAR_CSR5_REG &= (uint8)~POT_SAR_DLY_INC;
    #else /* Set High non-overlap delay for sampling clock signals (for <500KSPS)*/
        POT_SAR_CSR5_REG |= POT_SAR_DLY_INC;
    #endif /* POT_HIGH_POWER_PULSE == 0u */
    ...
    */
    #if(POT_HIGH_POWER_PULSE == 0u)    /* MinPulseWidth <= 50 ns */
        POT_SAR_CSR5_REG |= POT_SAR_SEL_CSEL_DFT_CHAR;
    #else /* for <500ksps */
        POT_SAR_CSR5_REG &= (uint8)~POT_SAR_SEL_CSEL_DFT_CHAR;
    #endif /* POT_HIGH_POWER_PULSE == 0u */

    /* Set default power and other configurations for control register 0 in multiple lines */
    POT_SAR_CSR0_REG = (uint8)((uint8)POT_DEFAULT_POWER << POT_SAR_POWER_SHIFT)
    /* SAR_HIZ_CLEAR:   Should not be used for LP */
    #if ((CY_PSOC5LP) || (POT_DEFAULT_REFERENCE != POT__EXT_REF))
        | POT_SAR_HIZ_CLEAR
    #endif /* SAR_HIZ_CLEAR */
    /*Set Convertion mode */
    #if(POT_DEFAULT_CONV_MODE != POT__FREE_RUNNING)      /* If triggered mode */
        | POT_SAR_MX_SOF_UDB           /* source: UDB */
        | POT_SAR_SOF_MODE_EDGE        /* Set edge-sensitive sof source */
    #endif /* POT_DEFAULT_CONV_MODE */
    ; /* end of multiple line initialization */

    POT_SAR_TR0_REG = POT_SAR_CAP_TRIM_2;

    /* Enable clock for SAR ADC*/
    POT_SAR_CLK_REG |= POT_SAR_MX_CLK_EN;

    CyDelayUs(10u); /* The block is ready to use 10 us after the enable signal is set high. */

    #if(POT_IRQ_REMOVE == 0u)
        /* Clear a pending interrupt */
        CyIntClearPending(POT_INTC_NUMBER);
    #endif   /* End POT_IRQ_REMOVE */
...
void POT_Stop(void)
{
    ...
    POT_SAR_CSR0_REG &= (uint8)~POT_SAR_SOF_START_CONV;
    /* Disable the SAR ADC block in Active Power Mode */
    POT_PWRMGR_SAR_REG &= (uint8)~POT_ACT_PWR_SAR_EN;
    /* Disable the SAR ADC in Standby Power Mode */
    POT_STBY_PWRMGR_SAR_REG &= (uint8)~POT_STBY_PWR_SAR_EN;

    /* This is only valid if there is an internal clock */
    #if(POT_DEFAULT_INTERNAL_CLK)
        POT_PWRMGR_CLK_REG &= (uint8)~POT_ACT_PWR_CLK_EN;
        POT_STBY_PWRMGR_CLK_REG &= (uint8)~POT_STBY_PWR_CLK_EN;
    #endif /* End POT_DEFAULT_INTERNAL_CLK */
...
void POT_SetPower(uint8 power)
{
    ...
    power &= POT_SAR_API_POWER_MASK;

    /* Set Power parameter  */
    tmpReg = POT_SAR_CSR0_REG & (uint8)~POT_SAR_POWER_MASK;
    tmpReg |= (uint8)(power << POT_SAR_POWER_SHIFT);
    POT_SAR_CSR0_REG = tmpReg;
}


/*******************************************************************************
* Function Name: POT_SetResolution
********************************************************************************
*
* Summary:
...
void POT_SetResolution(uint8 resolution)
{
    ...
    switch (resolution)
    {
        case (uint8)POT__BITS_12:
            tmpReg = POT_SAR_RESOLUTION_12BIT | POT_SAR_SAMPLE_WIDTH;
            break;
        case (uint8)POT__BITS_10:
            tmpReg = POT_SAR_RESOLUTION_10BIT | POT_SAR_SAMPLE_WIDTH;
            break;
        case (uint8)POT__BITS_8:
            tmpReg = POT_SAR_RESOLUTION_8BIT | POT_SAR_SAMPLE_WIDTH;
            break;
        default:
            tmpReg = POT_SAR_RESOLUTION_12BIT | POT_SAR_SAMPLE_WIDTH;
    ...
    POT_SAR_CSR2_REG = tmpReg;

     /* Calculate gain for convert counts to volts */
    POT_CalcGain(resolution);
}
    ...
    /*******************************************************************************
    * Function Name: POT_StartConvert
    ********************************************************************************
    *
    * Summary:
    ...
    *  POT_StartConvert(). This function is not available when the
    ...
    *******************************************************************************/
    ...
    {
        #if(POT_DEFAULT_CONV_MODE != POT__FREE_RUNNING)  /* If software triggered mode */
            POT_SAR_CSR0_REG &= (uint8)~POT_SAR_MX_SOF_UDB;   /* source: SOF bit */
        #endif /* End POT_DEFAULT_CONV_MODE */

        /* Start the conversion */
        POT_SAR_CSR0_REG |= POT_SAR_SOF_START_CONV;
    }


    /*******************************************************************************
    * Function Name: POT_StopConvert
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    void POT_StopConvert(void)
    {
        ...
        POT_SAR_CSR0_REG &= (uint8)~POT_SAR_SOF_START_CONV;
            ...
            POT_SAR_CSR0_REG |= POT_SAR_MX_SOF_UDB;    /* source: UDB */
        #endif /* End POT_DEFAULT_CONV_MODE */
...
/*******************************************************************************
* Function Name: POT_IsEndConversion
********************************************************************************
*
* Summary:
...
*   POT_RETURN_STATUS: Immediately returns the status. If the
...
*   POT_WAIT_FOR_RESULT: Does not return a result until the ADC
...
uint8 POT_IsEndConversion(uint8 retMode)
{
    ...
    do
    {
        status = POT_SAR_CSR1_REG & POT_SAR_EOF_1;
    } while ((status != POT_SAR_EOF_1) && (retMode == POT_WAIT_FOR_RESULT));
    /* If convertion complete, wait until EOF bit released */
    if(status == POT_SAR_EOF_1)
    {
...
*  POT_IsEndConversion() should be called to verify that the data
*   sample is ready.
*
* Parameters:
*  None.
...
*
* Global Variables:
*  POT_shift - used to convert the ADC counts to the 2s
*  compliment form.
*
* Side Effects:
...
{
    return( (int8)POT_SAR_WRK0_REG - (int8)POT_shift);
}


/*******************************************************************************
* Function Name: POT_GetResult16
********************************************************************************
*
* Summary:
...
int16 POT_GetResult16( void )
{
    ...
    res = CY_GET_REG16(POT_SAR_WRK_PTR);

    return( (int16)res - POT_shift );
}


/*******************************************************************************
* Function Name: POT_SetOffset
********************************************************************************
*
* Summary:
*  Sets the ADC offset, which is used by POT_CountsTo_Volts(),
*  POT_CountsTo_mVolts(), and POT_CountsTo_uVolts()
...
*
* Global Variables:
*  The POT_offset variable modified. This variable is used for
*  offset calibration purpose.
*  Affects the POT_CountsTo_Volts,
*  POT_CountsTo_mVolts, POT_CountsTo_uVolts functions
...
*
* Global Variables:
*  POT_shift variable initialized. This variable is used to
*  convert the ADC counts to the 2s compliment form.
*  POT_countsPer10Volt variable initialized. This variable is used
...
{
    int32 counts;
    #if(!((POT_DEFAULT_RANGE == POT__VSS_TO_VREF) || \
         (POT_DEFAULT_RANGE == POT__VSSA_TO_VDDA) || \
         (POT_DEFAULT_RANGE == POT__VSSA_TO_VDAC)) )
        uint16 diff_zero;
    #endif /* End POT_DEFAULT_RANGE */

    switch (resolution)
    {
        case (uint8)POT__BITS_12:
            counts = (int32)POT_SAR_WRK_MAX_12BIT;
            #if(!((POT_DEFAULT_RANGE == POT__VSS_TO_VREF) || \
                 (POT_DEFAULT_RANGE == POT__VSSA_TO_VDDA) || \
                 (POT_DEFAULT_RANGE == POT__VSSA_TO_VDAC)) )
                diff_zero = POT_SAR_DIFF_SHIFT;
            #endif /* End POT_DEFAULT_RANGE */
            break;
        case (uint8)POT__BITS_10:
            counts = (int32)POT_SAR_WRK_MAX_10BIT;
            #if(!((POT_DEFAULT_RANGE == POT__VSS_TO_VREF) || \
                 (POT_DEFAULT_RANGE == POT__VSSA_TO_VDDA) || \
                 (POT_DEFAULT_RANGE == POT__VSSA_TO_VDAC)) )
                diff_zero = POT_SAR_DIFF_SHIFT >> 2u;
            #endif /* End POT_DEFAULT_RANGE */
            break;
        case (uint8)POT__BITS_8:
            counts = (int32)POT_SAR_WRK_MAX_8BIT;
            #if(!((POT_DEFAULT_RANGE == POT__VSS_TO_VREF) || \
                 (POT_DEFAULT_RANGE == POT__VSSA_TO_VDDA) || \
                 (POT_DEFAULT_RANGE == POT__VSSA_TO_VDAC)) )
                diff_zero = POT_SAR_DIFF_SHIFT >> 4u;
            #endif /* End POT_DEFAULT_RANGE */
            break;
    ...
    POT_countsPerVolt = 0; /* Clear obsolete variable */
    /* Calculate gain in counts per 10 volts with rounding */
    POT_countsPer10Volt = (((counts * POT_10MV_COUNTS) +
                        POT_DEFAULT_REF_VOLTAGE_MV) / (POT_DEFAULT_REF_VOLTAGE_MV * 2));
...
*
* Global Variables:
*  POT_countsPer10Volt variable modified. This variable is used
*  for gain calibration purpose.
*
*******************************************************************************/
void POT_SetGain(int16 adcGain)
{
    POT_countsPer10Volt = (int32)adcGain * 10;
}


/*******************************************************************************
* Function Name: POT_SetScaledGain
********************************************************************************
*
* Summary:
...
{
    POT_countsPer10Volt = adcGain;
}


/*******************************************************************************
* Function Name: POT_CountsTo_mVolts
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  POT_offset variable used.
*  POT_countsPer10Volt variable used.
*
*******************************************************************************/
int16 POT_CountsTo_mVolts(int16 adcCounts)
{
    ...
    if(POT_countsPerVolt != 0)
    {   /* Support obsolete method */
        countsPer10Volt = (int32)POT_countsPerVolt * 10;
    }
    else
    {
        countsPer10Volt = POT_countsPer10Volt;
    }

    /* Subtract ADC offset */
    adcCounts -= POT_offset;
    /* Convert to millivolts with rounding */
    mVolts = (int16)( (( (int32)adcCounts * POT_10MV_COUNTS ) + ( (adcCounts > 0) ?
...
/*******************************************************************************
* Function Name: POT_CountsTo_uVolts
********************************************************************************
*
* Summary:
...
* Global Variables:
*  POT_offset variable used.
*  POT_countsPer10Volt used to convert ADC counts to uVolts.
*
*******************************************************************************/
int32 POT_CountsTo_uVolts(int16 adcCounts)
{
    ...
    */
    uVolts = (( (int64)adcCounts * POT_10UV_COUNTS ) / countsPer10Volt);
...
/*******************************************************************************
* Function Name: POT_CountsTo_Volts
********************************************************************************
*
* Summary:
...
* Global Variables:
*  POT_offset variable used.
*  POT_countsPer10Volt used to convert ADC counts to Volts.
*
*******************************************************************************/
float32 POT_CountsTo_Volts(int16 adcCounts)
{
    ...
    volts = ((float32)adcCounts * POT_10V_COUNTS) / (float32)countsPer10Volt;
