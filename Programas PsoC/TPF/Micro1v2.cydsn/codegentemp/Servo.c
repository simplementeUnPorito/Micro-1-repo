...
void Servo_SetDriveMode(uint8 mode)
{
	...
	CyPins_SetPinDriveMode(Servo_1, mode);
	CyPins_SetPinDriveMode(Servo_2, mode);
	CyPins_SetPinDriveMode(Servo_3, mode);
}
...
/* If interrupt is connected for this Pins component */ 
    ...
    *******************************************************************************/
    void Servo_SetInterruptMode(uint16 position, uint16 mode)
    {
		...
		if((position & Servo_1_INTR) != 0u) 
		{ 
			 Servo_1_INTTYPE_REG = (uint8)mode; 
		} 
		if((position & Servo_2_INTR) != 0u) 
		{ 
			 Servo_2_INTTYPE_REG = (uint8)mode; 
		} 
		if((position & Servo_3_INTR) != 0u) 
		{ 
			 Servo_3_INTTYPE_REG = (uint8)mode; 
		}
    }
